...
export function DestinationCard({ destination, className }: DestinationCardProps) {
  const localImage = getDestinationCardImage(destination.name, destination.country);
  const image = useDestinationPhoto(localImage, destination.name, destination.country);
