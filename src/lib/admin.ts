...
export type GroundHandlingPartnerRow = {
  id: string;
  name: string;
  airportCode: string;
  country: string | null;
...
function mapGroundHandlingPartnerRow(row: Record<string, unknown>): GroundHandlingPartnerRow {
  return {
    id: String(row["id"]),
...
export async function adminCreateGroundHandlingPartner(
  ...
  const { data, error } = await supabase.rpc("admin_create_ground_handling_partner", {
    p_access_token: token,
    p_name: params.name,
    p_airport_code: params.airportCode,
    p_country: params.country ?? null,
    p_contact_email: params.contactEmail ?? null,
    p_contact_phone: params.contactPhone ?? null,
  });
  if (error) rpcError(error);
  return mapGroundHandlingPartnerRow(data as Record<string, unknown>);
}
...
export async function adminDeleteGroundHandlingPartner(token: string, id: string): Promise<void> {
  const { error } = await supabase.rpc("admin_delete_ground_handling_partner", {
    p_access_token: token,
    p_id: id,
  });
  if (error) rpcError(error);
...
export type GroundHandlingRequestRow = {
  requestId: string;
  bookingId: string;
  ticketCode: string | null;
  serviceName: string;
...
function mapGroundHandlingRequestRow(row: Record<string, unknown>): GroundHandlingRequestRow {
  return {
    ...
    partnerName: (row["partner_name"] as string | null) ?? null,
...
export async function adminListGroundHandlingRequests(
  token: string,
  status?: string | null,
): Promise<GroundHandlingRequestRow[]> {
  const { data, error } = await supabase.rpc("admin_list_ground_handling_requests", {
    p_access_token: token,
    p_status: status ?? null,
  });
  if (error) rpcError(error);
  return ((data ?? []) as Record<string, unknown>[]).map(mapGroundHandlingRequestRow);
}
