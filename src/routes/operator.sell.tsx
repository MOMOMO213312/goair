...
import { Banknote, CheckCircle2, ClipboardPaste, CreditCard, FileSpreadsheet, Lock, Users } from "lucide-react";
...
import { formatOperatorMoney, getOperatorDashboard, isOperatorAuthError } from "@/lib/operator";
import {
  createBookingSafe,
...
export const Route = createFileRoute("/operator/sell")({
  head: () => ({
    meta: [
      { title: "بيع لعميلك — بوابة شركة النقل" },
      { name: "description", content: "احجز رحلة نقل مطار مباشرة لعميلك الخاص — تتحوّل تلقائيًا لأسطولك." },
      { name: "robots", content: "noindex" },
    ],
  }),
  component: OperatorQuickSalePage,
});
...
/** Common header labels operators might leave in the first row/column of their sheet. */
...
function OperatorQuickSalePage() {
  ...
  if (dashboardQuery.isPending) return <OperatorLoading />;
  if (dashboardQuery.isError || !dashboardQuery.data) {
    return isOperatorAuthError(dashboardQuery.error) ? <OperatorAuthError /> : <OperatorAuthError message="حصل خطأ مؤقت." />;
  }

  const { salesReferralCode, pendingSettlementUsd, pendingSettlementCount } = dashboardQuery.data;

  if (!salesReferralCode) {
    return (
      <OperatorSection title="بيع لعميلك">
        <p className="text-sm text-muted-foreground">
          حسابك لسه مش مربوط بكود بيع مباشر — كلم فريق GoAir عشان يفعّل الميزة دي لشركتك.
        </p>
      </OperatorSection>
    );
  }
  ...
  return (
    <QuickSaleForm
      ...
      pendingSettlementUsd={pendingSettlementUsd}
...
type CollectionMethod = "goair_gateway" | "collected_by_operator";
...
function QuickSaleForm({
  ...
  const [collectionMethod, setCollectionMethod] = useState<CollectionMethod>("goair_gateway");
  ...
  const [result, setResult] = useState<{ ticketCode: string; collectedByOperator: boolean } | null>(null);
  ...
  const scheduleQuery = useQuery({
    queryKey: ["operator-sell-schedule", tripId, date],
  ...
  const privateOptionsQuery = useQuery({
    queryKey: ["operator-sell-private-options", tripId],
    ...
    try {
      const { ticketCode } = isPrivate
        ? await createPrivateBookingSafe({
            ...
            passengerNames,
            paymentCollection: collectionMethod,
          })
      ...
      setResult({ ticketCode, collectedByOperator: collectionMethod === "collected_by_operator" });
  ...
  if (result) {
    return (
      <OperatorSection title="تم إنشاء الحجز">
        <p className="text-sm text-muted-foreground">
          كود التذكرة: <span className="font-display text-lg font-extrabold text-primary">{result.ticketCode}</span>
        </p>
        {result.collectedByOperator ? (
          <p className="mt-2 text-sm text-muted-foreground">
            الحجز اتسجل وهيتحوّل تلقائيًا لأسطولك. بما إنك استلمت الفلوس من عميلك بنفسك، قيمة الحجز دي هتتضاف
            لمستحقاتك المعلّقة اللي مديون بيها لـ GoAir، وتقدر تتابعها من صفحة "كشوف الحساب".
          </p>
        ) : (
          <p className="mt-2 text-sm text-muted-foreground">
            الحجز اتسجل وهيتحوّل تلقائيًا لأسطولك. أكمل الدفع من صفحة{" "}
            <a href={`/payment?ticket=${result.ticketCode}`} target="_blank" rel="noopener noreferrer" className="font-bold text-accent hover:underline">
              الدفع
            </a>{" "}
  ...
  return (
    <OperatorSection
      title="بيع لعميلك"
      description="هتتحوّل تلقائيًا لأسطولك — من غير ما تعدّي على طابور توزيع الرحلات العام."
    >
      {pendingSettlementCount > 0 ? (
        <div className="mb-6 rounded-lg border border-dashed border-accent/40 bg-accent/5 p-3 text-sm">
          <span className="font-bold text-primary">مستحق عليك لـ GoAir حاليًا: </span>
          <span className="font-bold text-accent">{formatOperatorMoney(pendingSettlementUsd)}</span>
          <span className="mr-1 text-xs text-muted-foreground">
            {" "}
            — من {pendingSettlementCount} حجز استلمت فلوسه بنفسك ولسه معلّق. تقدر تتابعها من "كشوف الحساب".
          </span>
        </div>
      ) : null}
      ...
      <div className="mb-6 rounded-lg border border-border/80 bg-secondary/20 p-4">
        <p className="mb-3 text-sm font-bold text-primary">طريقة تحصيل الفلوس</p>
        <RadioGroup
        ...
        >
          <label
            className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 transition-colors ${
              collectionMethod === "goair_gateway" ? "border-accent bg-accent/5" : "border-border"
            }`}
          >
            <RadioGroupItem value="goair_gateway" id="cm-gateway" className="mt-0.5" />
            <span>
              <span className="flex items-center gap-1.5 font-bold text-primary">
                <CreditCard className="size-4" aria-hidden /> الدفع من خلال GoAir
              </span>
              <span className="mt-1 block text-xs text-muted-foreground">
                عميلك بيدفع أونلاين من صفحة الدفع بتاعة GoAir زي أي حجز عادي.
              </span>
            </span>
          </label>
          <label
            className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 transition-colors ${
              collectionMethod === "collected_by_operator" ? "border-accent bg-accent/5" : "border-border"
            }`}
          >
            <RadioGroupItem value="collected_by_operator" id="cm-operator" className="mt-0.5" />
            <span>
              <span className="flex items-center gap-1.5 font-bold text-primary">
                <Banknote className="size-4" aria-hidden /> استلمت الفلوس بنفسي
              </span>
              <span className="mt-1 block text-xs text-muted-foreground">
                كاش أو تحويل مباشر من عميلك — القيمة هتتسجل كمستحق عليك لـ GoAir تتسوّاه لاحقًا.
              </span>
            </span>
          </label>
        </RadioGroup>
